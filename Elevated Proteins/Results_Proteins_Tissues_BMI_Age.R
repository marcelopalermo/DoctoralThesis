...
df_plot_Age_BMI = read.csv(file = '/Users/marcelo_palermo/Doutorado/R/Olink/NextSteps/Results/Top10_proteins_tissue_flattened_Age_BMI.csv', sep = ',')

df_plot_biomarkers_tissues_aggregated = rbind(df_plot_biomarkers_tissues_aggregated,df_plot_Age_BMI)

p<- ggballoonplot(
  df_plot_biomarkers_tissues_aggregated, x = "Model", y = "Tissue",
  size = 2.5, fill = "Total",
  facet.by = c("Split","SigProt"), 
  ggtheme = theme_bw(base_size = 7.5) 
) +  guides(size = FALSE) +  gradient_fill(c("lightyellow", "orange", "palevioletred1")) + #+ scale_fill_viridis_c(option = "C") + 
  geom_text(aes(label=Total), alpha=1.0, size=2.0, nudge_x = 0.35, nudge_y =0) 




#plot(ggpar(p,title = paste0("Predicted biomarkers - ", ifelse(mode == "Age_BMI","E(B|A)","E(A|B)"))))
plot(ggpar(p,title = paste0("Predicted biomarkers")))

write.csv(df_plot_biomarkers_tissues_aggregated,paste0("/Users/marcelo_palermo/Doutorado/R/Olink/NextSteps/Results/Top10_proteins_tissue_flattened_complete.csv"), row.names=TRUE)
