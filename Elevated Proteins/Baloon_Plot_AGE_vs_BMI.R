...
#proteins = read.csv('Protein_Elevation_Charts_Age_BMI_65_35.csv')
proteins = read.csv(paste0('Protein_Elevation_Charts_Age_BMI_',split_factor_file,'.csv'))
...
p <- ggballoonplot(df, x = "BMI", y = "Age", size = "Significant.Protein.elevation",
...
#ggpar(p,title = "Train: 65%, Testing: 35%",legend.title = "Number of\nelevated proteins")
...
p<- ggballoonplot(df, x = "BMI", y = "Age", 
...
proteins = read.csv('Protein_Elevation_Charts_BMI.csv')
df <- as.data.frame(proteins)
head(df)
p<- ggballoonplot(df, x = "Age", y = "BMI", 
                  size = "Significant.Protein.elevation", fill = "Significant.Protein.elevation") +
  scale_fill_viridis_c(option = "C") +
  guides(size = FALSE) + labs(title="Number of COVID-19 elevated proteins per Age based on BMI")+
  # move the title text to the middle
  theme(plot.title=element_text(hjust=0.5))
ggpar(p,legend.title = "Number of\nelevated proteins")
